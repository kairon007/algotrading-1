...
candle_start_times={"1","16","31","46"}
candle_start_seconds={"1"}

while(datetime.now().minute in candle_start_times and datetime.now().second in candle_start_seconds):
	aStocksList=[]
	pivots={}
	...
	sym_count=0
	tabularData=[("symbol","closingtime","open","close","high","low","bodysize","wicksize","bodypercentage","closepercentage","candlesize","candlecolor")]
	tabularData2=[("symbol","Time","SMA10","Breakout Signal","Breakout Conviction","Reversal signal","Reversal Conviction")]
	for stock in aStocksList:
		...
		if(process_sym=="" or sym_count==0):
			...
			if(pivot_record):
				if((cur_candle[3]>pivot_record.H4) and cur_candle[8]>0.75):
					h4Breakout="buy"
				if((cur_candle[3]<pivot_record.L4) and cur_candle[8]>0.75):
					l4Breakout="sell"
			...
			if(pivot_record):
				...
				if(h3ConvictionCounts >0):
					reversalConviction = str((h3ConvictionCounts*100)/3)+"%"
					reversalSignal="Sell"
				if(l3ConvictionCounts >0):
					reversalConviction = str((l3ConvictionCounts*100)/3)+"%"
					reversalSignal="Buy"

				if(h4ConvictionCounts >0):
					breakoutConviction=str((h4ConvictionCounts*100)/3)+"%"
					breakoutSignal="Buy"
				if(l4ConvictionCounts >0):
					breakoutConviction=str((l4ConvictionCounts*100)/3)+"%"
					breakoutSignal="Sell"
			...
			if(breakoutSignal!=" " or reversalSignal!=" "):
				...
				# print(tabulate(tabularData,headers="firstrow"))
				tempTabular = [process_sym,closing_time,process_sma10,breakoutSignal,breakoutConviction,reversalSignal,reversalConviction]
