...
for symbol in records_sym:
	...
	for record in records:
		...
		if(today=='' or today!=asiaTime.date()):
			...
			if(buy_trade == False and tradeCount ==0  and high_price >= (open_price+open_price*0.01)):
				'''
			...
			elif(buy_trade == False and tradeCount >0  and high_price >= (open_price+open_price*0.01) and high_price <= (open_price+open_price*0.0125)):
				'''
			...
			elif(buy_trade==True and high_price >= (open_price+open_price*0.0175)):
				...
				tabularData.append([str(buy_time),buy_price,sell_price,str(asiaTime),profit_perc,"Success"])
				...
				tabularData.append([str(buy_time),buy_price,sell_price,str(asiaTime),profit_perc,"Failed"])
